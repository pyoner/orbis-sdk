...
type ResponseIndexedDid = {
  status: number;
  did: string;
  result: {
    indexing_metadata: Record<string, string>;
    nonces: Record<string, number>;
  };
};

/** Force index a did to retrieve blockchain details such as nonces and ens name. */
export async function forceIndexDid(did: string) {
  const requestOptions = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
  ...
  let _result: ResponseIndexedDid;
  try {
    let _result_api = await fetch(
      "https://api.orbis.club/index-orbis-did/" + did,
      requestOptions
    );
...
export function buf2hex(buffer: Buffer) {
