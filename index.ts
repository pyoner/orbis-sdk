...
const postSchemaCommit =
  "k3y52l7qbv1fryn1brl9cvef0hilj561ooi6i3ac1l21rj2npron7nf2xneuace0w";
const channelSchema =
  "k3y52l7qbv1fry19cf09hgx3lvrqjvqknpzj8ens7hh3xaz0pmu7kret3y7gqs1kw";
...
export class Orbis {
  ...
  async connect(provider?: EIP1193Provider) {
    ...
    let _userAuthSig = localStorage.getItem(
      "lit-auth-signature-" + addresses[0]
    );
  ...
  /** Update user profile */
  async updateProfile(content: Profile) {
    /** Create a new stream with those details */
    let result = await this.createTileDocument(
      content,
      ["orbis", "profile"],
      "k3y52l7qbv1frxkhi9rm5y7wh48sbs8u39jwx8ufgd443awu5s8y0xufnbprp5qm8"
    );
    return result;
  }
  ...
  async setNotificationsReadTime(type, timestamp) {
    ...
    let result = await this.createTileDocument(
      ...
      "k3y52l7qbv1frykd0d0oyarndssmdfyxnuwt8v6da20i7pubplopd76mzndsf6hhc"
    );
  ...
  async createPost(post: Post, encryptionRules?: EncryptionRules) {
    /** Make sure post isn't empty */
    if (!post || !post.body || post.body == "" || post.body == undefined) {
      return {
        status: 300,
    ...
    let content: Post | EncryptedPost = { ...post };
    /** Check if posts should be encrypted  */
    if (encryptionRules) {
      try {
        const encryptedBody = await encryptPost(encryptionRules, content.body);
        content = { ...content, body: undefined, encryptedBody };
      } catch (e) {
  ...
  async react(post_id: string, type: Reaction) {
    /** Require post_id */
    ...
    let result = await this.createTileDocument(
      content,
      ["orbis", "reaction"],
      "k3y52l7qbv1frxuh68tbxjkp88bc37jc7p0thipi36nhhpgtb7r2ekimtudfbt9ts"
    );
    return result;
  }

  /** Users can create or update a new group which can be used as a context when sharing posts */
  async createGroup(content: Group) {
    /** Try to create a new Orbis group stream */
    let result = await this.createTileDocument(
      content,
      ["orbis", "group"],
      "k3y52l7qbv1frxlke0x7wquudb9q00d6fsatbobub6in3yvim5x6r9cnjbi0i0phc"
    );
    ...
    if (result.doc) {
      ...
      let channel_content: Channel = {
        name: "general",
        type: "feed",
      };

      /** Create a new stream for the channel */
      let channel_result = await this.createChannel(
        result.doc,
        channel_content
      );
  ...
  async createChannel(group_id: string, content: Channel) {
  ...
  async updateChannel(channel_id: string, content: Channel) {
    if (!channel_id) {
  ...
  async setGroupMember(group_id: string, active = true) {
    ...
    let result = await this.createTileDocument(
      content,
      ["orbis", "group_member"],
      "k3y52l7qbv1frxy7omau0nf3w6mlc482iqog83e5utihk7pntqfpvpiintkiosg74"
    );
    return result;
  }

  /** Users can follow other users */
  async setFollow(did: string, active = true) {
    /** Make sure group_id is available */
    if (!did) {
    ...
    let result = await this.createTileDocument(
      content,
      ["orbis", "follow"],
      "k3y52l7qbv1fry1x6vtd1396nvf1nljn62vfirvol5deahbl41rygx6ceo0cex9ts"
    );
    return result;
  }

  /** User can update a group */
  async updateGroup(stream_id: string, content: Group) {
    if (!stream_id) {
  ...
  async createConversation(content: Conversation) {
    ...
    let result = await this.createTileDocument(
      _content,
      ["orbis", "conversation"],
      "k3y52l7qbv1frypruvvdaho7nnkc5d2s0hqq0uokxbg9wga7mnqv8ci5bdv13latc"
    );
  ...
  async sendMessage(content: Message) {
    ...
    if (
      !conversation.content?.recipients ||
      conversation.content?.recipients.length <= 0
    ) {
      return {
        status: 300,
        error:
          "Recipients array is empty or doesn't exist. Please retry or create a new conversation.",
    ...
    try {
      _encryptedContent = await encryptDM(
        conversation.content.recipients,
        content.body
      );
    } catch (e) {
    ...
    let result = await this.createTileDocument(
      _content,
      ["orbis", "message"],
      "k3y52l7qbv1fry9kw12vvvn52a0x16asq599y4vl1tt42kjd1ype4pw5fwuj3dkao"
    );
    return result;
  }

  /** Decrypt an encrypted post using Lit Protocol */
  async decryptPost(content: EncryptedPost) {
    let res;
    try {
  ...
  async decryptMessage(content: EncryptedMessage) {
    let res;
    try {
   ...
   */
  async getPosts(options?: GetPostsOptions, page = 0) {
    let query;
          ...
          case "recommendations":
            query = this.api
              .rpc("orbis_recommendations", {
                user_did:
                  this.session && this.session ? this.session.id : "none",
              })
              .range(page * 50, (page + 1) * 50 - 1);
            break;
          case "all-posts":
  ...
  async getIsGroupMember(group_id: string, did: string) {
    let res = false;
    let { data, error, status } = await this.api
  ...
  async getConversations(options?: GetConversationsOptions) {
    ...
    if (options?.context && options?.did) {
      ...
      return {
        data: [],
        error: "The `did` is required when retrieving conversations.",
        status: 300,
      };
    }
